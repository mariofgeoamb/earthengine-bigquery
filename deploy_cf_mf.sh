...
cf_rainfall="polyrainfallcf-gen2"
...
endpoint_rainfall=$(gcloud functions describe ${cf_rainfall} --region=${REGION} --gen2 --format=json | jq -r '.serviceConfig.uri')
...
build_sql="CREATE OR REPLACE FUNCTION gee.get_poly_rainfall_month(aoi STRING, year int64, month int64) RETURNS STRING REMOTE WITH CONNECTION \`${project_id}.us.gcf-ee-conn\` OPTIONS ( endpoint = '${endpoint_rainfall}')"
...
bq load --source_format=CSV --replace=true --skip_leading_rows=1  --schema=aoi:STRING,name:STRING ${project_id}:gee.farms_coords_MT_RS  ./fazendas_RS_MT.csv

sleep 60

#bq query --use_legacy_sql=false 'SELECT gee.get_poly_ndvi_month(farm_aoi,2020,7) as ndvi_jul FROM `gee.farms_coords_MT_RS` LIMIT 10'
bq query --use_legacy_sql=false 'SELECT * from `gee.farms_coords_MT_RS` LIMIT 10'
